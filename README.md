...
- **镜头连续性** — `scene_id` 只区分物理地点；只有无剪切、边界景别兼容的直接续拍才使用 `seamless`，同场景插入特写和换机位仍会有意切镜
- **参考职责分离** — 无缝续接只用真实尾帧作为 `first_frame`；有意切镜只用角色锚点作为 `reference_image`，避免身份与起始状态相互污染
- **漂移边界** — 连续尾帧链最多 2 次，之后通过有意切镜重新使用角色参考图；极端特写和遮挡镜头不能作为角色锚点
...
连续 seamless 最多 2 次，避免输出尾帧逐段累积角色和场景漂移。
