...
# 可选：限制本次运行允许提交的付费视频 Take 数量
python main.py "制作一个30秒的视频，机器人在末日城市清除丧尸" --paid-take-budget 8
```
...
- `--paid-take-budget N` 为本次运行设置新的付费 Take 上限。预算在提交前预留并写入 `run_manifest.json`；远端任务轮询不消耗新预算，语义重拍会消耗新的 Take 授权。
- 长序列由生产计划安排 canonical identity intentional cut；只有已验收并提取成功的身份锚点存在时才重置 `Reference Chain Depth`，否则继续使用尾帧，不以牺牲连续性换取形式上的重锚定。
...
离线行为评测矩阵覆盖 15/30 秒、action/product/balanced、单/多角色、同/跨场景、四种交互模式和 normal/policy-safe 编译；不会调用真实 LLM、视频或付费服务。
