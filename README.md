...
- **分镜契约校验 + 自动修正** — 检查稳定场景 ID、景别、情绪、物件/时序、主题、prompt、时长、单镜动作负载、角色移动形态和动作空间轴，严重问题只触发一次 LLM 修正
- **题材自适应节奏** — 根据用户需求区分动作驱动、产品展示与均衡叙事；动作片让运镜服务攻防，产品片保留细节 Insert Shot，明显失衡时只修正一次
...
- **可拍摄性契约** — `coverage_role`、必需可见主体和交互几何共同约束同框、攻击线与遮挡；拒绝“极近景浅焦却要求远处目标清晰命中”等互相冲突的镜头
